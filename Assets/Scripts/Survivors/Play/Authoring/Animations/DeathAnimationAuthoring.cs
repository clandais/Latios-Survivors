...
namespace Survivors.Play.Authoring.Animations
{
	public class DeathAnimationAuthoring : MonoBehaviour
	{
		...
		[SerializeField]  DeathClips deathAnimation;
		
		

		[TemporaryBakingType] struct AnimationClipSmartBakeItem : ISmartBakeItem<DeathAnimationAuthoring>
		{

			SmartBlobberHandle<SkeletonClipSetBlob> m_clipHandle;
			
			public bool Bake(DeathAnimationAuthoring authoring, IBaker baker)
			{
				Entity entity = baker.GetEntity(TransformUsageFlags.Dynamic);
				baker.AddComponent<Components.DeathClips>(entity);
			...
			public void PostProcessBlobRequests(EntityManager entityManager, Entity entity)
			{
				...
				entityManager.SetComponentData(entity, new Components.DeathClips
				{
					ClipSet = clipset
				});
			}
		...
		private class DeathAnimationBaker : Baker<DeathAnimationAuthoring>
		{
